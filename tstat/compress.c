...
static FILE *
CompSaveHeader (char *filename, struct comp_formats *pf)
{
  FILE *f_stream;
  FILE *f_file;
  char buf[COMP_HDR_SIZE];
...
static int
CompOpenPipe (char *filename, struct comp_formats *pf)
{
  int fdpipe[2];
  char *abspath;
  int i;
...
/* v1.2.0 ---------------------------------------------------------------*/
struct comp_formats *CompFormat;
char comp_buffer[512];
char *
CompGetCommand (char *filename)
{
  if (CompFormat == NULL)
    {
      return NULL;
    ...
    {
      int i;
      sprintf (comp_buffer, "%s ", CompFormat->comp_bin);
      for (i = 1; CompFormat->comp_args[i + 1] != NULL; i++)
	{
	  strcat (comp_buffer, CompFormat->comp_args[i]);
	  strcat (comp_buffer, " ");
	}
      strcat (comp_buffer, filename);
      return comp_buffer;
    }
}
...
CompOpenHeader (char *filename)
{
  ...
  if (FileIsStdin (filename))
    {
      is_compressed = FALSE;	/* pretend that it not compressed */
      return (stdin);
    }

  /* see if it's a supported compression file */
  CompFormat = pf = WhichFormat (filename);
...
static void
PipeFitting (FILE * f_pipe, FILE * f_header, FILE * f_orig_stdin)
{
