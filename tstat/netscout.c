...
static int
pread_netscout (struct timeval *ptime,
		int *plen,
		int *ptlen,
		void **pphys, int *pphystype, struct ip **ppip, void **pplast)
{
  ...
  *pplast = (char *) pip_buf + 48;
  ...
  sscanf (strlen ("Time") + strstr (buffer_string, "Time"),
	  "%s %d %d:%d:%d.%d", month, &day, &hour, &minute, &sec, &msec);
  tmval.tm_sec = sec;
  tmval.tm_min = minute;
  tmval.tm_hour = hour;
  tmval.tm_mday = day;
  ...
  *pphystype = PHYS_ETHER;
  ...
  pep_buf[index++] =
    ntohl (byte12 << 24 | byte13 << 16 | byte14 << 8 | byte15);
