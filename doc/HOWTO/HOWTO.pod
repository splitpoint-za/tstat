...
and if it was found the C<Librrd> in the system in which case is automatically
...
Libtstat-demo is only a demostration tool so is build only a local
...
C<193.204.134.0/24>, we can choose to use one of the following formats:
...
can be specified usin C<'#'> so using following
...
that can be usefull for generic elaborations. In any case, if the C<-N> option
...
The C<-H> has to be used to enable the histogram engine, specifing a configuration
...
Furthermore the following log files are created: C<log_mm_complete> for multimedia flows
...
became to bigs, and to dinamically change the set of packet level traces to collect. 
...
Except from C<snap_len> all features are on/off type that is 0 and 1 are used to 
enable/disable them. Instead C<snap_len> indicate the maximum number of bytes
dump from the input packet starting from IP level included. For example, supposing
to have a simple IP + UDP packet, using C<snap_len> = 40 it will be writed a
packet of 54 bytes

    14 (bogus ethernet) + 20 (IP) + 8 (UDP) + 12 (payload)

Note that, a change
in the dump configuration means the generation of a new C<tracesNN> output directory
and for this reason, inside the packet level traces directories is placed a log file
called C<log.txt> that indicate the configuration related to the output 
...
and specifing C<PKTSIZE> we will describe packet size feature.
...
to avoid underflows because of logaritm function (by default 1e-33);
...
different gaussian distributions. Each function is defined by mean and sandard deviaton sigma
...
enabled simply specifing the C<-B> command line option:
...
Insted C<ip_direction> indicates if the packet is incoming or outgoing. Possible values :
...
1: packet if treated as having internal source and internal destination (local packet)
