...
Tstat is tested on C<Linux> systems (currently C<Ubuntu>, C<Debian>, C<RedHat>, 
and C<CentOS>, using C<2.x> and C<3.x> kernels), and on C<Mac OS X> (starting 
from 10.6 C<Snow Leopard> to the current 10.10 C<Yosemite>). 
It includes support for compilation for C<Android>, and has been reported working on 
C<OpenWRT>. 
It should work under C<FreeBSD>, C<NetBSD>, and other C<unix-like> systems, 
...
Assuming that you want version C<3.x.y>:
...
=item <Network IPv4/MaskLen> on a single line 

    130.192.0.0/16
    192.204.134.0/16
                                                         

=item <Network IPv6/MaskLen> on a single line 

    2100:abcd:ef00::/40
    fe80::/64
                                                         

=item <Network IPv4/NetMask IPv4> on a single line 

    130.192.0.0/255.255.0.0
    192.204.134.0/255.255.0.0
                             

=item (only for backward compatibility) pairs of lines with <Network IP> and <NetMask IPv4>
...
support has been enabled at compilation time (as reported by C<tstat -v>).
...
encryption process using the C<-W> command line option with a file in the same format 
used for the C<-N> option. Addresses satisfying both the C<-Y> and the C<-W> lists 
will be whitelisted and will appear B<unencrypted> in the log files.
...
IPv6 addresses are currently not obfuscated (C<-A>).
...
might be considered sensitive, i.e. the URLs information in C<log_http_complete> and 
the YouTube VideoIDs in C<log_video_complete>. The command line option C<-0> makes Tstat 
run in privacy mode: in this mode, the runtime.conf option C<httplog_full_url> is forced 
to 0, so that full URLs cannot be logged in C<log_http_complete>, and the YouTube 
Video IDs in C<log_video_complete> are masked as C<'XXX'>.

Finally, logs can contain information about the contacted servers, in the form of the SNI and
the certificate information for TLS connections, the DNS name for the contacted server,
and the Hostname for plain text HTTP connections. This information normally is included 
in C<log_tcp_complete> when Layer7 logging is enabled, and in C<log_udp_complete>.
When address encryption is enabled (C<-Y>), using the command line option C<--enc> 
all hostnames are encrypted and Base64-encoded before being written in the logs.
For the sake of simplicity, the Rijndael ECB cryptographical function used by Crypto-PAn
and the first 16 bytes of the key provided for C<-Y> are used: this makes the solution weak
from the cryptographic point of view, so other forms of encryption might be needed in more 
sensitive scenarios.
...
C<tstat-conf/globals.conf>. We report here a simplified list of the available parameters.
    ...
    crypto_ipv6_mask_0 = 0xffffffff	# Mask used when encrypting dword 0 of IPv6 addresses
    crypto_ipv6_mask_1 = 0xf0f0f0f0	# Mask used when encrypting dword 1 of IPv6 addresses
    crypto_ipv6_mask_2 = 0x0f0f0f0f	# Mask used when encrypting dword 2 of IPv6 addresses
    crypto_ipv6_mask_3 = 0x00000000	# Mask used when encrypting dword 3 of IPv6 addresses
    
Parameter names are case-insensitive. Values in the C<[globals]> section can have integer or 
floating point value. Integer values can also be provided in hexadecimal format, prefixing 
the value with '0x'. Types are loosely enforced, i.e. integer values can be provided to 
floating point parameters, but not the opposite. A warning or an error message is generated 
when the value or the type of a parameter is not correct or cannot be safely converted to 
the right type.
